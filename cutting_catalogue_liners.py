...
def in_field(field):
    ...
    file1 = 'stages_values_for_master_catalogues_hst.txt'
    ...
    ### Now we add a new cut based on the OSIRIS and photometric redshifts using the total aperture measurements. 
    z_cut = np.where((np.isnan(MC_z) == False) & ((MC_z - z_median_total) <= (0.888*MC_z - 0.1358))  & (MC_z > 0.133) & (MC_z <= 0.195))[0]
    
    ### One line aper5
    cut_one_line_5 = np.where((error_ewha_median_5[z_cut]/(ewha_median_5[z_cut]) < 0.15) & (Prob_posiew_5[z_cut] > 99.7) & (oneline_5[z_cut] == 1))[0]
    
    ### One line total aper
    cut_one_line_total = np.where((error_ewha_median_total[z_cut]/(ewha_median_total[z_cut]) < 0.15) & (Prob_posiew_total[z_cut]  > 99.7) & (oneline_total[z_cut]  == 1))[0]

    ###Two lines aper5
    cut_two_lines_5 = np.where((error_ewha_median_5[z_cut]/(ewha_median_5[z_cut]) < 0.15) & (Prob_posiew_5[z_cut]  > 99.7) & (twolines_5[z_cut]  == 1))[0]
    
    ###Two lines total aper
    cut_two_lines_total = np.where((error_ewha_median_total[z_cut]/(ewha_median_total[z_cut]) < 0.15) & (Prob_posiew_total[z_cut]  > 99.7) & (twolines_total[z_cut]  == 1))[0]
    
    ###Two lines total aper more restricted
    cut_two_lines_total_01 = np.where((error_ewha_median_total[z_cut]/(ewha_median_total[z_cut]) < 0.1) & (Prob_posiew_total[z_cut]  > 99.7) & (twolines_total[z_cut]  == 1))[0]
        
    ### Two lines and AGN
    cut_two_lines_agn_5 = np.where((error_ewha_median_5[z_cut]/(ewha_median_5[z_cut]) < 0.15) & (Prob_posiew_5[z_cut]  > 99.7) & (twolines_5[z_cut]  == 1) & (prob_agn5[z_cut]  > 99.7))[0]
    
    ### Two lines and SF
    cut_two_lines_sf_5 = np.where((error_ewha_median_5[z_cut]/(ewha_median_5[z_cut]) < 0.15) & (Prob_posiew_5[z_cut]  > 99.7) & (twolines_5[z_cut]  == 1) & (prob_sf5[z_cut]  > 99.7))[0]
    
    f_zcut = f[z_cut]


    ### Catalogue with all the galaxies within the z cut in the total aperture measurement
    np.savetxt('F'+str(field)+'_catalogue_z_cut_total_master_hst_liners.txt',f[z_cut],fmt=formato)


    ### Catalogue with all the galaxies within the two lines cut in the total aperture measurement
    np.savetxt('F'+str(field)+'_catalogue_cut_two_lines_total_master_hst_liners.txt',f_zcut[cut_two_lines_total],fmt=formato)

    ### Catalogue with all the galaxies within the two lines cut in the total aperture measurement MORE RESTRICTED
    np.savetxt('F'+str(field)+'_catalogue_cut_two_lines_total_01_master_hst_liners.txt',f_zcut[cut_two_lines_total_01],fmt=formato)
    
    
    ### Catalogue with all the galaxies within the two lines cut in the 5 pixel aperture measurement
    np.savetxt('F'+str(field)+'_catalogue_cut_two_lines_aper5_master_hst_liners.txt',f_zcut[cut_two_lines_5],fmt=formato)
    
    ### Catalogue with all the galaxies within the one line cut in the total aperture measurement
    np.savetxt('F'+str(field)+'_catalogue_cut_one_line_total_master_hst_liners.txt',f_zcut[cut_one_line_total],fmt=formato)
    
    
    
    f1 = open('number_of_galaxies_hst_liners.txt','a+')
    f1.write('%2i %3i %3i %3i %3i %3i \n'%(field,len(field_number), len(z_cut),len(cut_one_line_total),len(cut_two_lines_total), len(cut_two_lines_5)))
    f1.close()
