...
def get_data(glx,field):
    ...
    #filename='../plot_aper/plot_aper_web/spectrum_gal_'+str(glx)+'_aper5.txt'
    #filename='../plot_aper/F'+str(field)+'/F'+str(field)+'_spectrum_gal_'+str(glx)+'_aper5.txt'
    filename='../plot_aper/all_spectra_hst/F'+str(field)+'_spectrum_gal_'+str(glx)+'_aper5_hst.txt'
...
######## AUTOCORRELATION CHECKS!!!!!! #####################
    ...
    plt.savefig('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/F'+str(field)+'_mcmc_burnin_acor_'+str(glx)+'.pdf')
   ...
   ### Autocorrelation for AFTER the burn-in
    ...
    ax.set_ylabel(r"Autocorrelation")
    #ax.figure.savefig("acor.png", transparent=True, dpi=300)
    plt.savefig('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/F'+str(field)+'_mcmc_acor_'+str(glx)+'.pdf')
    ...
    f1 = open('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/F'+str(field)+'_gal_'+str(glx)+'_convergence_test.txt','a+')
    ...
    ax = plt.figure().add_subplot(111)
    ax.plot(all_R_k)
    plt.savefig('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/F'+str(field)+'_gal_'+str(glx)+'_convergence_test.pdf')

   
##################### END OF AUTOCORRELATION CHECKS!!!!!!! #################################    

    
    f1 = open('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/F'+str(field)+'_acceptance_fractions.txt','a+')
    f1.write('%5i %.3f\n'%(glx,np.mean(sampler_fixha_pt.acceptance_fraction)))
    f1.close()

    print("Mean acceptance fraction: {0:.3f}"
                    .format(np.mean(sampler_fixha_pt.acceptance_fraction)))
   ...
   # print 'prob', -2*prob_reshaped
    ...
    hdulist.writeto('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/table_'+str(glx)+'.fits')
    '''
    ...
    for i, p in enumerate(par):
        ...
        plt.savefig('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/mcmc_runs'+str(glx)+'.pdf')
    plt.close()
    ...
    for i, p in enumerate(par):
        ...
        plt.xlabel(p)
    plt.legend()    
    plt.savefig('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/mcmc_runs_histograms_'+str(glx)+'.pdf')    
    plt.close()
    ...
    plt.savefig('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/mcmc_runs_fits_'+str(glx)+'.pdf')    
    ...
    plt.errorbar(xmeans, ymeans, error_flux, None, 'o')
    plt.savefig('../mcmc_fits/galaxy_pages_aper5/F'+str(field)+'/mcmc_runs_triangle_'+str(glx)+'.pdf')    
    '''
